<template>
    <Head title="Editar Topping" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Editar Topping</h2>
        </template>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <form @submit.prevent="submit">
                            ...
                            <div class="mb-4">
                                <label for="imagen" class="block text-3xl font-medium text-gray-700 dark:text-gray-300">Imagen</label>
                                <input 
                                    type="text" 
                                    v-model="form.imagen" 
                                    class="mt-1 block w-full" 
...
const validateField = (field) => {
    switch (field) {
        ...
        case 'cantidad':
            if (form.cantidad < 0) {
                errors.value.cantidad = 'La cantidad debe ser mayor que -1.';
            } else {
        ...
        case 'imagen':
            if (form.imagen <= 0) {
                errors.value.imagen = 'La imagen no puede ser un campo vacío.';
            } else {
                errors.value.imagen = '';
            }
            break;
